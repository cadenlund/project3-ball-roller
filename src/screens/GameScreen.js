...
export function GameScreen({ levelId, onExit, onFinish }) {
  ...
  const size = Math.min(width - 32, height - 260);
  ...
  return (
    <View style={styles.screen}>
      ...
      <Board level={level} state={view} size={size} />
      ...
      <View style={styles.footer}>
        ...
        <Text style={styles.source}>
          {source === 'gyro' ? 'tilt to roll' : 'arrow keys / WASD to roll'}
        </Text>
      </View>

      {/* On-screen pad so the game is playable where there is no gyro and no keyboard. */}
      {source !== 'gyro' && (
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: '#020617', alignItems: 'center', paddingTop: 64 },
  hud: {
    width: '100%', paddingHorizontal: 22, marginBottom: 18,
  ...
  footer: { marginTop: 18, alignItems: 'center', gap: 4 },
  meta: { color: '#94a3b8', fontSize: 14 },
  source: { color: '#475569', fontSize: 12 },
  pad: { position: 'absolute', bottom: 36, width: 168, height: 168 },
  padBtn: {
